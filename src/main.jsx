...
import { ArrowUpRight, Bot, CalendarDays, CloudSun, Compass, ExternalLink, KeyRound, MapPin, Menu, Mountain, RefreshCw, Send, Settings, Sparkles, Users, X } from 'lucide-react'
import './styles.css'
...
async function askGemini(apiKey, question, tours, weather, history) {
  const context = `You are Coast, the friendly travel concierge for Atlantic Coast Tours, a boutique Irish operator. Answer using only the live catalogue and weather tool results below. Be concise, warm, and practical. Use euro prices exactly as supplied. If something is unavailable, say so. Never invent availability. Mention that weather is a forecast and conditions can change.\n\nLIVE TOUR CATALOGUE:\n${JSON.stringify(tours)}\n\nLIVE WEATHER:\n${JSON.stringify(weather)}\n\nConversation:\n${history.map((m) => `${m.role}: ${m.text}`).join('\n')}\n\nCustomer: ${question}`
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${encodeURIComponent(apiKey)}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: context }] }], generationConfig: { temperature: 0.55, maxOutputTokens: 500 } }) })
...
function App() {
  const [key, setKey] = useState(() => localStorage.getItem('act-gemini-key') || '')
  ...
  const submit = async (event, preset) => {
    ...
    if (!key) { setMessages((old) => [...old, { role: 'user', text: question, time: 'Now' }, { role: 'assistant', text: 'Add your Gemini API key in the settings panel above to start chatting with me.', time: 'Now' }]); setInput(''); return }
    setInput(''); setLoading(true); setMessages((old) => [...old, { role: 'user', text: question, time: 'Now' }])
    try { const nextWeather = await getWeather(question); setWeather(nextWeather); const answer = await askGemini(key, question, tours, nextWeather, messages); setMessages((old) => [...old, { role: 'assistant', text: answer, time: 'Now' }]) }
    catch (error) { setMessages((old) => [...old, { role: 'assistant', text: error.message, time: 'Now', error: true }]) }
    finally { setLoading(false) }
  }

  return <div className="app-shell">
    <header className="topbar"><a className="brand" href="#top"><span className="brand-mark"><Compass size={20} /></span><span>Atlantic Coast <b>TOURS</b></span></a><nav className={menuOpen ? 'open' : ''}><a href="#chat">Ask Coast</a><a href="#experiences">Experiences</a><a href="#about">Our Ireland</a></nav><div className="top-actions"><span className="live-pill"><i /> Live data</span><button className="settings-btn" onClick={() => setShowKey(true)}><Settings size={15} /> {key ? 'AI ready' : 'Connect AI'}</button><button className="menu-btn" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={20} /> : <Menu size={20} />}</button></div></header>
    <main id="top">
      ...
      <section className="status-strip"><div><span className="strip-icon"><CloudSun size={18} /></span><span><b>Local forecasts</b><small>{weather ? `Updated for ${weather.location}` : 'Open-Meteo weather on request'}</small></span></div><div className="ai-status"><KeyRound size={15} /> <span>{key ? 'AI connected' : 'AI not connected'}</span></div></section>
    ...
    </main>{showKey && <div className="modal-backdrop" onClick={() => setShowKey(false)}><div className="settings-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setShowKey(false)}><X size={18} /></button><span className="modal-icon"><KeyRound size={20} /></span><p className="eyebrow">Private browser setting</p><h3>Connect your AI guide</h3><p>Paste your Google AI Studio key here to let Coast answer questions. It stays in this browser.</p><label>Gemini API key<input autoFocus type="password" value={key} onChange={(e) => { setKey(e.target.value); localStorage.setItem('act-gemini-key', e.target.value) }} placeholder="AIza..." /></label><button className="primary-btn modal-save" onClick={() => setShowKey(false)}>Save key <ArrowUpRight size={16} /></button><small className="security-note">For production, use a server-side proxy so the key is never exposed to visitors.</small></div></div>}<footer id="about"><span className="brand"><span className="brand-mark"><Compass size={16} /></span> Atlantic Coast <b>TOURS</b></span><span>West is best. Galway, Ireland · © 2025</span><a href={`https://docs.google.com/spreadsheets/d/${SHEET_ID}/edit`} target="_blank" rel="noreferrer">View live source <ExternalLink size={13} /></a></footer>
  </div>
}
