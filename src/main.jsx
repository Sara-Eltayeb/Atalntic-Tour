...
import { ArrowUpRight, Bot, CalendarDays, ChevronDown, CloudSun, Compass, ExternalLink, KeyRound, MapPin, Menu, MessageCircle, Mountain, RefreshCw, Send, Sparkles, Users, X } from 'lucide-react'
import './styles.css'
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main id="top">
      ...
      <section className="status-strip"><div><span className="strip-icon"><RefreshCw size={17} /></span><span><b>Live catalogue</b><small>{catalogueState === 'live' ? `${tours.length} experiences synced from Google Sheets` : catalogueState === 'loading' ? 'Syncing the latest trips...' : 'Showing our latest trip list'}</small></span></div><div><span className="strip-icon"><CloudSun size={18} /></span><span><b>Local forecasts</b><small>{weather ? `Updated for ${weather.location}` : 'Open-Meteo weather on request'}</small></span></div><div className="key-control"><KeyRound size={16} /><input aria-label="Gemini API key" type={showKey ? 'text' : 'password'} value={key} onChange={(e) => { setKey(e.target.value); localStorage.setItem('act-gemini-key', e.target.value) }} placeholder="Paste Gemini API key" /><button onClick={() => setShowKey(!showKey)}>{showKey ? 'Hide' : 'Show'}</button></div></section>
